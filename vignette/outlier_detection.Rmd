...
knitr::include_graphics(here::here("vignette/figures", "diagram_quality_control.jpg"))
```
## Data set
...
weather <- purrr::map(unique(weather$doy), ~ get_longterm_mean_and_sd(weather = weather,
                                                           variable = 'Tmin',
                                                           doy = .x)) %>%
  bind_rows() %>%
...
anomalies <- purrr::map(aux_list, function(x){
  purrr::map(unique(x$doy), ~ get_longterm_mean_and_sd(weather = x,
                                                       variable = 'Tmin',
                                                       doy = .x)) %>%
    bind_rows() %>%
    merge(x, by = 'doy') %>%
