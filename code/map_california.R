...
sites_ca <- st_as_sf(aux_overview, coords = c("Longitude", "Latitude"), 
...
map_california <- tm_shape(elevation, bbox = extent(-125, -114, 32.5, 42.5)) +
  ...
  tm_shape(sites_ca) +
  tm_symbols(size = 0.3, shape = 'source',col = 'source', 
             palette=c('CIMIS'='red', 'UCIPM'='grey70'),
             shapes = c(25,21), legend.col.show = FALSE, legend.shape.show = F)+
...
tmap_save(map_california, "figures/map_california_qc.png", width = 17.6, height = 23.4, units = "cm", dpi = 600)
