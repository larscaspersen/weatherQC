...
library(gridExtra, quietly = T) #for side by side plots
...
I realized that some cleaning is necessary before doing further analysis (like feeding the data to weather generators or calculating chill portions). Since manual outlier detection requires time and skill (both of which I do not possess), so automated outlier detection routines are necessary. There are some general data quality packages specialised mainly for medical data @marino_r_2022 but few are specialized for weather data. The patching and homogenisation package 'Climatol' offers a simple outlier detection where the weather reading is compared to a model output @guijarro_homogenization_2021. Based on a (user-defined) threshold daily measurements with a residual (|observation - model|) exceeding the threshold were identified and removed. The reddprec package is specialized on quality control and missing data imputation of precipitation data. A detailed set of weather outlier detection algorithm is described by @durre_comprehensive_2010, who also validated the algorithms performance by manually checking the labelled outliers. Instead of using a single criterion a set of different plausibility checks were carried out. The plausibility checks range from basic integrity tests (for example duplicated records in different months) to more detailed tests like spatial integrity checks. These algorithms were also used for the Global Historical Climatology Network (GHCN-) Daily and are allegedly also available as a FORTRAN-code, but I can't find in on the web. A less exhaustive set of outlier tests were proposed by @costa_gap_2021 for a data set of Brazilian weather stations. Both proposed weather quality algorithms of @costa_gap_2021 and @durre_comprehensive_2010 were here reconstructed in R as close as possible from the descriptions in the manuscripts. While the output of both function is very similar, `weather_QC_costa` and `weather_qc_durre` differ in their testing principles. The scheme after Costa uses a parallel testing concept, erroneous data is then flagged by the convergence of at least two tests. In contrast, `weather_qc_durre` has a subsequent testing design: the testing order is important and the test become more and more specific. There, a single positive test result leads to the flagging of the observation. 


```{r test_design, fig.align = 'center', echo=F, out.width = "50%", fig.cap = "Parallel test design of weather_qc_costa (left) and subsequent test design of weather_qc_durre (right).", label = 'tetst_design'}
knitr::include_graphics(here::here("vignettes/figures", "diagram_quality_control.jpg"))
```

## Data set

In this document I will demonstrate the functions and the tests within the weather quality control scheme at the example of one weather station in California and 12 neighbouring weather stations within a 74 km range. The data is retrieved from CIMIS and UCIPM weather station data base via the R package `chillR`. The data used in this script can be accessed with the `weatherQC` package. The target weather station data set includes daily weather observation from 1990 to then end of 2021 for minimum, maximum and mean temperature (Tmin, Tmax, Tmean) and precipitation (Precip). 

```{r map_stations_california, fig.align = 'center', echo=F, out.width = "50%", fig.cap = "Map of target stations (from CIMIS database) and auxiliary weather stations for spatial plausibility checks (from UCIPM and CIMIS database)", label = 'map_stations_california'}
knitr::include_graphics(here::here("vignettes/figures", "map_california_qc.png"))
```
...
#run costa 2021 quality control
costa_test_result <- read.csv('qc_costa_example.csv')
...
durre_test_result <- read.csv('qc_durre_example.csv')
...
The next section is about the spatial consistency of the observations. The first test in this section is a regression check of temperature observation called `spatial_consistency_test()`. This test only applies to Tmax and Tmin. At first only neighboring stations within a 75km radius around the target station were considered. Using a three day window centered on the day of interest, a weighted mean of the pairwise regression of target and neighbor values is calculated. Target neighbor regressions needed to have correlation coefficient of 0.8 or larger in order to be considered. At least three, but never more than seven neighboring stations were used for the regression. Mean value of the regression is calculated based on the index of agreement. Regressions are done for each year / month independently. In order to be flagged, the residual and the standardized residual must exceed a threshold. (>= 4 and 8, respectively).
...
x <- weatherQC:::select_target_days(df = weather, variable = variable, period_start = period_start, period_end = period_end) %>%
  unname()


#take the 7 closest stations
aux_info$dist <- sp::spDistsN1(pts = as.matrix(aux_info[, c('Longitude', 'Latitude')]), 
                              pt = c(target_info$Longitude,
                                           target_info$Latitude),
                              longlat = TRUE)
#order increasing distance
aux_info <- aux_info[order(aux_info$dist),]

#select 7 closest stations
aux_info <- aux_info[1:7,]
...
y <- purrr::map(aux_list, function(x){
  weatherQC:::select_target_days(df = x, variable = variable, period_start = period_start, 
...
aux_info$ind_agreement <- purrr::map_dbl(y, ~ weatherQC:::calc_index_agreement(x = x, y = .x))
...
y_closest <-  purrr::map(y, function(vec) purrr::imap_dbl(x,~weatherQC:::get_closest_y(x = .x, y=vec, i = .y))) %>%
  do.call(cbind.data.frame, .)
...
The dashed lines show the tesing threshold. Both need to be exceeded in order for an observation to be flagged. We can have also a look at the observation of the neighboring stations which led to the flagging, even though the lineplot already shows that the flagged osbervation is fishy.
...
#prepare data.frame
aux_Tmin <- purrr::map(aux_list, function(x){
...
aux_Tmin <-  merge.data.frame(aux_Tmin, aux_info, by.x = 'variable', by.y = 'id', all.x = T)


#add info of target station and coordinates to

aux_Tmin <- rbind(aux_Tmin, 
                  data.frame(day = c(-1,0,1), variable = target_info$id, 
...
The auxiliary weahter stations are marked by dots, the target weather station by an white-filled diamond. As you can see, the target observation of 0 degree C deviates strongly from the neighboring observations of the precious day (day -1), the same day (day 0) and the following day (day +1). 

The spatial regression has relatively high quality demands on the target - neighbor station data quality. In cases were the requirements were not met, the second test can fill a gap. The corroboration test of temperature `test_temperature_corroboration()` and precipitation `test_precipitation_spatial_corroboration()` are less demanding on the data coverage and thus can be used in cases the spatial regression test cannot be applied. The temperature corroboration test calculates for each observed day the temperature anomaly to the standardized long-term mean For the lowest absolute difference calculation a three day window centered on the day of interested is used for the neighboring station. That means if there are seven neighbor stations having for each of the three day window a valid observation, the target observation of day 0 is compared to 21 neighboring observations. The three-day time window should account for different measurement protocols. If the smallest absolute difference is larger or equal to 10°C, then the target observation is flagged. In case of precipitation, the absolute difference accompanied by the difference in climatological percentiles. Climatological percentiles are calculated following the same protocoll described for the climatological outliers. The difference is, that this time the percentile of the values of interest are calculated and not compared to a fixed threshold. The smallest absolute difference in climatololoigical percentiles is then used to determine a testing threshold for the smallest absolute difference in precipitation. For more details please refer to the Appendix C of @durre_comprehensive_2010 

```{r, warning=FALSE}

sum(durre_test_result$flag_Tmin == 'spatial_corroboration', na.rm = T)

```

There are several cases of positive test results.  

```{r, warning=FALSE}

durre_test_result %>%
  filter(Month == 1, Year == 1992, 
         flag_Tmin %in% c('gap_check', 'record_exceedance', 'iterative_consistency',
                          'spatial_regression') == F) %>%
  ggplot(aes(x = Date, y = Tmin_org))+
  geom_point() +
  geom_line() + 
  ylab('Minimum Temperature (°C)')+
  geom_point(data = durre_test_result[durre_test_result$Month == 1 &
                                        durre_test_result$Year == 1992 & 
                                        (durre_test_result$flag_Tmin ==
                                           'spatial_corroboration'),], 
             aes(x = Date, y = Tmin_org), 
             size = 7, pch = 1, stroke = 2, col = 'red')
...
#just take the day 10 as an example

#prepare weather data so that it is in the same state as in the function call
weather <- durre_test_result %>%
  mutate(Tmin = Tmin_org)

#remove any outlier detected until spatial regression
weather$Tmin[weather$flag_Tmin %in% c('gap_check', 'record_exceedance', 'iterative_consistency', 'spatial_regression')] <- NA


#### calculate longterm mean & anomaly ####

#calculate anomaly for target weather station
weather <- purrr::map(unique(weather$doy), ~ weatherQC:::get_longterm_mean_and_sd(weather = weather,
                                                           variable = 'Tmin',
                                                           doy = .x)) %>%
  bind_rows() %>%
  merge(weather, by = 'doy') %>%
  arrange(Date) %>%
  mutate(anomaly = Tmin - mean)

target_anomaly <- weather$anomaly[weather$Year == 1992 & weather$Month == 1 & weather$Day == 10]

aux_list <- neighbour_weather[neighbour_info$id]

#calculate anomaly for auxiliary weather stations
anomalies <- purrr::map(aux_list, function(x){
  purrr::map(unique(x$doy), ~ weatherQC:::get_longterm_mean_and_sd(weather = x,
                                                       variable = 'Tmin',
                                                       doy = .x)) %>%
    bind_rows() %>%
    merge(x, by = 'doy') %>%
    arrange(Date) %>%
    mutate(anomaly = Tmin - mean) %>%
    filter(Year == 1992, Month == 1, Day %in% 9:11)
}) %>%
  bind_rows() %>%
  select(anomaly, doy)

anomalies$id <- rep(names(aux_list), each = 3)

#add latitude and longitude
anomalies <- merge(anomalies, neighbour_info, by = 'id', all.x = T)

#add target observation
anomalies <- rbind(anomalies, data.frame(id = target_info$id, Longitude = target_info$Longitude,
           Latitude = target_info$Latitude, Name = target_info$Name,
           anomaly = target_anomaly, doy = 9:11) ) %>%
  mutate(doy = factor(doy, levels = 9:11, labels = c('day -1', 'day 0', 
                                                     'day +1')))

anomalies %>%
  ggplot(aes(x = Longitude, y = Latitude, label = round(anomaly,2)))+
  geom_point() +
  ggrepel::geom_text_repel()+
  facet_wrap(~doy, ncol = 1, nrow = 3) +
  geom_point(data = anomalies[anomalies$id == 'cimis_2',], aes(x = Longitude,
                                                                  y = Latitude), shape = 23, size = 2, fill = "white", stroke = 1.2)+
  theme_bw()
...
show minimum absolute difference to neighbour against anomaly threshold
...
target_anomaly <- weather$anomaly[weather$Year == 1992 & weather$Month == 1]
...
anomalies <- purrr::map(aux_list, function(x){
  purrr::map(unique(x$doy), ~ weatherQC:::get_longterm_mean_and_sd(weather = x,
    ...
    arrange(Date) %>%
    mutate(anomaly = Tmin - mean) %>%
    mutate(lag = lag(anomaly), lead = lead(anomaly)) %>%
    filter(Year == 1992, Month == 1) %>%
    select(lag, anomaly, lead)
}) %>%
  bind_cols() %>%
  as.matrix()

#add target anomalies to column
anomalies <- cbind(target_anomaly, anomalies)

min_diff <- apply(anomalies, MARGIN = 1, function(x){
  #if target anomaly is NA, then return NA
  if(is.na(x[1])){
    return(NA) 
  #if all auxiliary observations are NA, return NA as well
  } else if(all(is.na(x[2:(length(x))]))){
    return(NA)
  } else{
    return(min(abs(x[1] - x[2:(length(x))]), na.rm = T))
  }
})

plot_df <- data.frame(day = rep(1:31,3),  value = c(weather$Tmin[weather$Year == 1992 & weather$Month == 1], target_anomaly, min_diff),
                      var = c(rep('Tmin', 31), rep('anomaly', 31), rep('min_abs_dif', 31))) %>%
  mutate(var = factor(var, levels = c('Tmin', 'anomaly', 'min_abs_dif')))

#dataframe for vertical line
line_df <- data.frame(value = c(NA, NA, 10), var = c('Tmin', 'anomaly', 'min_abs_dif')) %>%
  mutate(var = factor(var, levels = c('Tmin', 'anomaly', 'min_abs_dif')))

#dataframe for annotation
text_df <- data.frame(day = 1, value = 10, 
                      var = factor('min_abs_dif', levels =  c('Tmin', 'anomaly', 'min_abs_dif')),
                      lab = 'Testing Threshold')

plot_df %>%
  ggplot(aes(x = day, y = value )) + 
  geom_point() +
  geom_line() +
  facet_wrap(~var, nrow = 3, ncol = 1, scales = 'free_y') + 
  geom_hline(data = line_df, aes(yintercept = value), linetype = 'dashed')+
  geom_text(data = text_df, label = 'Testing Threshold',
            nudge_x = 2, nudge_y = 2.5)+
...
sum(durre_test_result$flag_Tmin == 'mega_consistency', na.rm = T)

```

No cases of positive test results for the megaconsistency test.


## Summary

No you have seen the different tests withing `weather_qc_costa` and `weather_qc_durre` and hopefully a better feeling how the functions try to detect erroneous weather data.
For best outcomes include all auxiliary weather stations in `weather_qc_durre` as well as target weather stations. That way, erroneous weather observations in neighbouring stations are less likely to degrade the spatial consistency tests. In the next vignette I will work on a more detailed evaluation of both test results and compare it with quality flags supplied by CIMIS. 
